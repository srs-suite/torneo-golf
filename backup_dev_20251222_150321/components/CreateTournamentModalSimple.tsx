...
const tournamentSchemaCreate = z.object({
  ...
  custodian: z.string().max(100, 'Máximo 100 caracteres').optional(),
...
export function CreateTournamentModalSimple({ isOpen, onClose, onSuccess, tournament, clubId }: CreateTournamentModalProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      custodian: (tournament as any)?.custodian || '',
          ...
          >
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Custodio - En posesión de */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Dinero en posesión de
                  </label>
                  <input
                    type="text"
                    {...register('custodian')}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-transparent"
                    placeholder="Ej: Tesorero, Caja del club..."
                  />
                  <p className="mt-1 text-xs text-gray-500">Quién tiene el dinero cobrado del torneo</p>
                  {errors.custodian && (
                    <p className="mt-1 text-sm text-red-600">{errors.custodian.message}</p>
                  )}
                </div>
